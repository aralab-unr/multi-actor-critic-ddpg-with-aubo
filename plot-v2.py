...
paths = ['/home/adarshsehgal/AACHER_logs/multi-ac-logs']
for curr_path in paths:
    if not os.path.isdir(curr_path):
        continue
    results = load_results(os.path.join(curr_path, 'progress.csv'))
    ...
    if env_id not in data2:
        data2[env_id] = {}
    if config not in data2[env_id]:
        data2[env_id][config] = []
    data2[env_id][config].append((x, y))
...
for env_id in sorted(data.keys()):
    ...
    for env_id2 in sorted(data2.keys()):
        for config in sorted(data2[env_id].keys()):
            xs, ys = zip(*data2[env_id][config])
            xs, ys = pad(xs), pad(ys)
            assert xs.shape == ys.shape
